...
def sample_nbeatsx_forecast():
    ...
    axes[0].fill_between(x=plot_df1['ds'][-12:], 
    ...
    axes[1].fill_between(x=plot_df2['ds'][-12:], 
...
def test_train_nbeatsx(ts: pd.DataFrame, shards: list, feat_cols: list):
    ...
    for shard in shards:
        ...
        for ts_len in df_train_val["ts_len"].unique():
            ...
            Y_train_df = (df_train_val[df_train_val["ts_len"] == ts_len]
                        .copy()
                        .drop(columns=["ts_len"]))
            Y_test_df = (df_test[df_test["ts_len"] == ts_len]
                        .copy()
                        .drop(columns=["ts_len"]))
            ...
            # Plot quantile predictions
            df_forecast_shard = (Y_hat_df.reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    ts.rename(columns={"Timestamp": "ds", 
                       'ts_key':'unique_id',
                       'Vol/Prod_ratio_kg':'y'}, inplace=True)
    ...
    feat_cols = ['trend_Lag_2',
    'sesonality_Lag_12',
    'Vol/Prod_ratio_kg_Rolling_Mean_6',
    'residuals_Rolling_Mean_6',
    'Vol/Prod_ratio_kg_Lag_2']
    
    df_feat_importance = pd.read_parquet('./data/gold/lgbm_covid_feat_importance.parquet')
